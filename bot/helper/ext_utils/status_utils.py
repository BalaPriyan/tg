...
async def get_readable_message(sid, is_user, page_no=1, status="All", page_step=1):
    ...
    for index, task in enumerate(
        ...
        msg += f"\n   by {task.listener.message.from_user.mention(style='html')} • ID{task.listener.message.from_user.id}"
        ...
        if (
            ...
            msg += f"\n   {task.speed()} • ETA: {task.eta()} • Elapsed: {get_readable_time(elapsed)}"
            ...
            msg += f"\n   {task.seed_speed()} • Ratio: {task.ratio()}"
            msg += f"\n   Seeding: {task.seeding_time()} • Elapsed: {get_readable_time(elapsed)}"
        else:
            msg += f"\n\n   Size: {task.size()}"
        msg += f"\n\n   Engine: {task.engine}"
        msg += f"\n   Mode: {task.listener.mode[0]} → {task.listener.mode[1]}"
        msg += f"\n   Status: <b>{tstatus}</b>"
